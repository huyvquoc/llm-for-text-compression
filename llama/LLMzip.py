...
class LLMzip_encode:
    ...
    def encode_from_tokens(
        ...
        for b_ind in range(n_batches):
            ...
            print(f'Encoder: Processing batch {b_ind+1} / {n_batches}')
        ...
        ret_metric_dict = self.compute_compression_ratio(tokens_encoded,probs_tok_full,starter_tokens)
    ...
    def compute_compression_ratio(self, tokens_encoded, probs_tok, starter_tokens):
      """
      Calculate and report compression metrics for encoded text.
      
      Computes various compression statistics including entropy bounds, compression ratios,
      and bit rates. Metrics are calculated differently based on the compression algorithm used.
      
      Args:
          tokens_encoded: NumPy array of token IDs that were compressed
          probs_tok: NumPy array of LLM-predicted probabilities for each token
          starter_tokens: Initial context tokens (if with_context_start was used), or None
      
      Returns:
          dict: Dictionary containing compression metrics:
              - N_C: Number of characters in original text
              - N_T: Number of tokens encoded
              - H_ub: Entropy upper bound (bits per character)
              - Token-by-token Bit rate: Theoretical bit rate using ceiling of -log2(prob)
              - Compressed file size: Actual compressed file size in bits
              - Compression ratio: Original size / Compressed size
              - Bit rate: Bits per character in compressed file
      """
      # Convert encoded tokens back to text to measure character count
      text_encoded = self.tokenizer.decode(tokens_encoded.squeeze().tolist())
      
      # Optional: Save starter text separately if needed for analysis
      ...
      N_T = tokens_encoded.size  # Total number of tokens
      N_C = len(text_encoded)    # Total number of characters
      ...
      # H_ub: Entropy upper bound - theoretical minimum bits needed per character
      # Calculated as: sum(-log2(p_i)) / N_C where p_i is probability of token i
      # This represents the cross-entropy between true distribution and LLM predictions
      df_out['H_ub'] = [str(np.sum(-1*np.log2(probs_tok))/N_C)]
      
      # =====================================================================
      # RankZip Compression Metrics
      # =====================================================================
      if (self.compression_alg == 'RankZip') or (self.compression_alg == 'both'):
          # Read the compressed RankZip file to measure actual size
          with open(self.RZ_file_name, 'rb') as file_RZ:
              ranks_compressed_bytes = file_RZ.read()
          
          # Calculate file sizes in bits
          compressed_size_bits = len(ranks_compressed_bytes) * 8  # Compressed file size
          original_size_bits = N_C * 8  # Original text size (8 bits per character)
          
          # rho: Bits per character in compressed file
          rho_RZ = compressed_size_bits / N_C
          
          # Compression ratio: how much smaller is compressed vs original
          # Higher is better (e.g., 2.0 means compressed is half the size)
          compression_ratio_RZ = original_size_bits / compressed_size_bits
          
          # Compression rate: inverse of compression ratio, expressed as bit rate
          compression_rate_RZ = 8/compression_ratio_RZ
          
          print(f'RankZip Compression Ratio: {compression_ratio_RZ:.4f}')
          print(f'RankZip Compression Rate: {compression_rate_RZ:.4f}%')
          
          # Token-by-token bit rate: theoretical rate if each token probability
          # is encoded independently using ceil(-log2(prob)) bits
          df_out['Token-by-token Bit rate'] = [str(np.sum(np.ceil(-1*np.log2(probs_tok)))/N_C)]
          df_out['Compressed file size'] = [compressed_size_bits]
          df_out['Compression ratio'] = [f'{compression_ratio_RZ:.4f}']
          df_out['Bit rate'] = [f'{compression_rate_RZ:.4f}']
      
      # =====================================================================
      # Arithmetic Coding Compression Metrics
      # =====================================================================
      if (self.compression_alg == 'ArithmeticCoding') or (self.compression_alg == 'both'):
          b_ind = 1  # Unused variable (legacy code)
          
          # Read the compressed arithmetic coding file as bitstream
          file_in = open(self.AC_file_name, 'rb')
          bitin = BitInputStream(file_in)
          compressed_bits = read_bitstream(bitin)
          
          # Calculate actual compressed size from bitstream
          compressed_size_bits = compressed_bits.size
          original_size_bits = N_C * 8  # Original text size (8 bits per character)
          
          # rho: Bits per character in compressed file
          rho_AC = compressed_size_bits / N_C
          
          # Compression ratio: original size / compressed size
          # Arithmetic coding typically achieves near-optimal compression
          # approaching the entropy bound H_ub
          compression_ratio_AC = original_size_bits / compressed_size_bits
          
          # Compression rate: bits per character in compressed file
          compression_rate_AC = 8/compression_ratio_AC
          
          print(f'AC Compression Ratio: {compression_ratio_AC:.4f}')
          print(f'AC Compression Rate: {compression_rate_AC:.4f}%')
          
          file_in.close()
          
          # Store metrics (same structure as RankZip for consistency)
          df_out['Token-by-token Bit rate'] = [str(np.sum(np.ceil(-1*np.log2(probs_tok)))/N_C)]
          df_out['Compressed file size'] = [compressed_size_bits]
          df_out['Compression ratio'] = [f'{compression_ratio_AC:.4f}']
          df_out['Bit rate'] = [f'{compression_rate_AC:.4f}']
