...
# BINARY_FILES=("cp.html" "kennedy.xls" "ptt5" "sum")
# BINARY_FILES=("ptt5" "sum")
BINARY_FILES=("kennedy.xls")
...
for filename in "${BINARY_FILES[@]}"; do
    ...
    if [ -f "$file" ]; then
        ...
        # Run torchrun
        CUDA_VISIBLE_DEVICES=6,7 torchrun --nproc_per_node 1 LLMzip_run.py \
